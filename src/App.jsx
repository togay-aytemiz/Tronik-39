...
*/
  ...
  const handleClick = (e) => {
    ...
    let targetKey = e.target.dataset.note; // "C4" gibi
    // console.log(targetKey);
    const updatedSynthKeys = synthKeys.map(synthKey => {
      if (synthKey.keyName === targetKey) {
        // Toggle the active state of the target key
        return {
          ...synthKey,
          active: !synthKey.active
        };
      }
      return synthKey;
    });

    setSynthKeys(updatedSynthKeys);

  }

  const handleKeyDown = (e) => {
    const keyPressed = e.key
    const pressedSynthKey = synthKeys.find(key => key.computerKey === keyPressed)
    console.log(keyPressed);

    if (pressedSynthKey && !pressedSynthKey.active) {

      const updatedSynthKeys = synthKeys.map(key => key === pressedSynthKey ? { ...key, active: true, keyPressed:true } : key);
      setSynthKeys(updatedSynthKeys);
    }
  }
  ...
  const handleKeyUp = (e) => {
    ...
    const releasedSynthKey = synthKeys.find(key => key.computerKey === releasedKey);

    if (releasedSynthKey) {

      const updatedSynthKeys = synthKeys.map(key => key === releasedSynthKey ? { ...key, active: false, keyPressed:false } : key);
      setSynthKeys(updatedSynthKeys);
    }
  }
  ...
  return (
    <div className='wrapper'>
      <div className='main-container'>
        ...
        <Keys showKeys={showKeys} synthKeys={synthKeys} onClick={handleClick} onKeyDown={handleKeyDown} onKeyUp={handleKeyUp}/>
      </div>
    </div>
  )
}
